...
export default function Layout({ children }: LayoutProps) {
  ...
  const title = (pathname ? pages[pathname] : undefined) ?? "Joblist";
    ...
    <SidebarProvider style={sidebarStyle}>
      ...
      <SidebarInset className="min-w-0  overflow-hidden bg-[#E8EFF3]">
        <header className="flex h-16 shrink-0 items-center gap-3 border-b border-[#0F766E]/20 bg-[#DCEBE9] px-4 text-[#0F172A] md:px-6">
          ...
          <span
            aria-hidden="true"
            className="h-5 w-px bg-[#0F766E]/25"
          />

          <span className="text-lg font-semibold">{title}</span>
        </header>

        <div className="flex-1 bg-[linear-gradient(135deg,#E0EEEB_0%,#E8EFF3_55%,#E8E9F5_100%)] p-4 md:p-6">
          {children}
        </div>
      </SidebarInset>
    </SidebarProvider>
